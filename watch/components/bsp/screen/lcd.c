...
{          
    ...
    for (uint16_t i = 0; i < height; i++) {
        // 调用你在 lcd_init.c 中实现的批量写函数
        // 如果没有实现，可以参考下方的 LCD_WR_DATA_BLOCK
        LCD_WR_DATA_BLOCK((uint8_t*)buffer, width * 2); 
    }
...
{
	// 1. 计算总字节数
    // 注意：xend, yend 如果是包含的坐标，则像素点 = (end - sta + 1)
    ...
    // 4. 批量发送
    // 这里建议检查一下 size*2 是否超过了你的 SPI DMA 最大限制
    // 如果 color_p 已经是交换过字节序的，直接发：
    LCD_WR_DATA_BLOCK((uint8_t*)color_p, size * 2);
