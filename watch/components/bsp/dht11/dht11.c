/**********************************************************************
 * 
 * RMT是ESP32系列芯片中的一个外设，支持高精度的时间控制和信号生成与接收。
 * 
 * rmt能应对处理好这种单总线协议通信的设备，而且不怕被打断：如果当前正在
 * CPU上运行获取接收DHT11的测量数据，ESP32不怕会被高优先级的任务打断从而
 * 切换任务影响数据接收，而是交给了RMT外设去干，保证接收的连续性和完整性。
 * 
 * *********************************************************************/
...
uint8_t DHT_Init(void)
{
    ...
    rmt_rx_channel_config_t rx_chan_config = {
        ...
        .resolution_hz = 1000 * 1000, 	  // 1 MHz 滴答分辨率，即 1 滴答 = 1 ?s
	...
	//创建rmt接收通道
	ret = rmt_new_rx_channel(&rx_chan_config, &rx_chan_handle);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "RMT RX channel creation failed");
        return 1;
    }
    ...
    ret = rmt_rx_register_event_callbacks(rx_chan_handle, &cbs, rx_receive_queue);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "RMT RX register event callbacks failed");
        return 1;
    }
    

	//使能RMT接收通道
	ret = rmt_enable(rx_chan_handle);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "RMT RX channel enable failed");
        return 1;
    }
...
uint8_t DHT_Read(int *humi, int *temp)
{
	...
	if (xQueueReceive(rx_receive_queue, &rx_data, pdMS_TO_TICKS(1000)) == pdTRUE) {
		...
		return parse_items(rx_data.received_symbols, rx_data.num_symbols,humi, temp);
	}
    return 0;
}
